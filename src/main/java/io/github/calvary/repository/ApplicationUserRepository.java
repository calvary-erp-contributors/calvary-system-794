...
public interface ApplicationUserRepository extends JpaRepository<ApplicationUser, Long>, JpaSpecificationExecutor<ApplicationUser> {
    ...
    @Query(
        value = "select distinct applicationUser from ApplicationUser applicationUser left join fetch applicationUser.createdBy left join fetch applicationUser.lastModifiedBy left join fetch applicationUser.systemIdentity",
    ...
    @Query(
        "select distinct applicationUser from ApplicationUser applicationUser left join fetch applicationUser.createdBy left join fetch applicationUser.lastModifiedBy left join fetch applicationUser.systemIdentity"
    )
    List<ApplicationUser> findAllWithToOneRelationships();

    @Query(
        "select applicationUser from ApplicationUser applicationUser left join fetch applicationUser.createdBy left join fetch applicationUser.lastModifiedBy left join fetch applicationUser.systemIdentity where applicationUser.id =:id"
    )
